	...
	<view class="page">
		...
		<view class="contentBox" v-if="contentShow" :style="{width:videoStyle.width,height:videoStyle.height}">
					...
					<view :style="{opacity:shadeNum > 2 ?'0':'1'}">
						<view class="vodPlayer">
							...
							:muted="index == vodIndex ? false : true"
							 :id="'myVideo'+index" @play="startPlay(index)" @waiting="bufferVod(index)" @timeupdate="timeupdateVod($event,index)" @error="errVod(index)" @ended="endedVod(index)">
							</video>
						</view>
						<!-- 视频封面 -->
						...
						<view class="vodPause-img" v-if="item.pauseShow">
							<image src="/static/icon/vodPause.png" class="vodPauseImage"></image>
						</view>
						<view @click="playSpot(index)" class="vodPlayView" :style="videoStyle" @touchstart="vodViewStart($event)"
			...
			<view class="flooter" :style="{opacity:moveOpacity ? 0 : 1,bottom:speedBottom+'px'}" v-if="vodList.length > 0 && beforeVodInfo.sliderShow">
				...
				<!-- 进度条 -->
				<view class="sliderBox" :style="{opacity:shadeNum > 2 ?'0':'1'}" @touchmove.stop="touchmoveSlider" @touchstart.stop="touchmoveSlider" @touchend.stop="touchendSlider" v-if="vodList.length > 0 && beforeVodInfo.sliderShow && videoTime > 0">
					...
					<view class="slider-item" :style="{height: '18px',opacity:brightSlider ? 1 : 0}">
						<!-- 底层 -->
						<view class="slider-box bright-box"></view>
						<!-- 滑块 -->
						<view class="slider-block bright-block" :style="{marginLeft:sliderProgress+'px'}"></view>
						<!-- 滑动层 -->
						<view class="slider-load bright-load" :style="{width:sliderProgress+'px'}"></view>
					</view>
				</view>
